...
describe('Flow Dapp Tests', async () => {
    ...
    describe('Fast Floward Tests', async () => {
        ...
        it(`removes a kittyitem from sale`, async () => {
            ...
            try {
                res1 = await DappLib.kittyItemsMarketRemoveMarketItem(testData1)
            } catch (e) {
                console.log(e)
            }
            ...
            assert.equal(res2.result, 1, "Incorrect length for SaleCollection for account[1]")

            // 8) assert the result (an array) from step 6) shows the SaleCollection has 
            // an itemID == 1 in it at index 0
            assert.equal(res3.result[0], 1, "Incorrect itemID for SaleCollection at index 0 for account[1]")
        })
        ...
        it(`checks both admin and user have correct balances after purchase`, async () => {
            ...
            assert.equal(res1.result, 5.0, "Amount not equal to 5.0")
            // 6) assert the result from step 4) equals 25.0
            assert.equal(res2.result, 25.0, "Amount not equal to 25.0")
        })
    });
