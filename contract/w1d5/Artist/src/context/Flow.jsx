      ...
      const transactionId = await fcl.send([
        fcl.transaction`
          ...
          transaction(listingIndex: Int) {
            ...
            execute {
              self.marketRef.withdraw(listingIndex: self.listingIndex, seller: self.seller)                 
            }        
        }
        `,
        fcl.args([
        ...
        fcl.transaction`
          ...
          transaction(listingIndex: Int) {
            ...
            prepare(account: AuthAccount) {
                ...
                let buyerVault <- account.load<@FlowToken.Vault>(from: /storage/flowTokenVault)!
                self.tokenVault <- buyerVault.withdraw(amount: self.marketRef.getListings()[listingIndex].price)                
                account.save<@FlowToken.Vault>(<- buyerVault, to: /storage/flowTokenVault)
                
            }                
            execute {
              self.marketRef.buy(listing: self.listingIndex, with: <- self.tokenVault!, buyer: self.buyer)                 
            }        
        }
        `,
        fcl.args([
